...
export function filterGlossaryEntries(entries: GlossaryEntryDto[], filter: string): GlossaryEntryDto[] {
  const query = searchable(filter);
  if (!query) return entries;
  return entries.filter((entry) => searchable([
    entry.sourceTerm,
...
export function recoverTranslationContextConflict(
  canonical: TranslationContextSnapshotDto,
  _replay?: () => void,
) {
  return {
    settings: {
      stylePreset: canonical.style,
      contextRevision: canonical.revision,
    },
...
export function TranslationSettingsPanelView({
  ...
  const visibleEntries = useMemo(() => filterGlossaryEntries(glossary, filter), [glossary, filter]);
  ...
  return (
    <section className="translation-settings" data-testid="translation-settings-panel" aria-label="Thiết lập dịch">
      <header className="translation-settings__header">
        <div>
          <span className="eyebrow">DỊCH THUẬT</span>
          <h3>Phong cách & thuật ngữ</h3>
        </div>
        ...
        {STYLE_OPTIONS.map((option) => (
          <label key={option.value} className={settings.stylePreset === option.value ? 'is-selected' : ''}>
            <input
              ...
              value={option.value}
              checked={settings.stylePreset === option.value}
      ...
      <div className="translation-settings__glossary-head">
        <div>
          <strong>Thuật ngữ dự án</strong>
          <span>{glossary.length} / 200</span>
        </div>
        <button type="button" className="ghost-button" disabled={saving || glossary.length >= 200} onClick={onStartCreate}>
          + Thêm
        </button>
      </div>

      <input
        className="translation-settings__search"
        ...
        <div className="translation-settings__draft">
          <label>
            ...
            <input
              ...
              maxLength={120}
              onChange={(event) => onDraftChange({ sourceTerm: event.currentTarget.value })}
            />
          </label>
          <label>
            <span>Bản dịch ưu tiên</span>
            <input
              value={draft.preferredTranslation}
              maxLength={200}
              onChange={(event) => onDraftChange({ preferredTranslation: event.currentTarget.value })}
            />
          </label>
          <label>
            <span>Ghi chú</span>
            <textarea
              ...
              maxLength={300}
              rows={2}
              onChange={(event) => onDraftChange({ note: event.currentTarget.value || null })}
            />
          </label>
          <label className="translation-settings__case-toggle">
            <input
              type="checkbox"
              checked={draft.caseSensitive}
              onChange={(event) => onDraftChange({ caseSensitive: event.currentTarget.checked })}
            />
      ...
      <div className="translation-settings__list">
        {visibleEntries.map((entry) => (
          <article key={entry.id} className="translation-settings__entry">
            <div>
              ...
              {entry.caseSensitive && <em>Phân biệt hoa thường</em>}
            </div>
        ...
        {!loading && visibleEntries.length === 0 && (
          <p className="translation-settings__empty">Chưa có thuật ngữ phù hợp.</p>
        )}
      </div>
...
export function TranslationSettingsPanel({ projectId, services = defaultServices }: TranslationSettingsPanelProps) {
  ...
  useEffect(() => {
    ...
    setConflictMessage('');
    Promise.all([
      services.loadTranslationSettings(projectId),
      services.loadGlossary(projectId),
    ]).then(([nextSettings, glossaryResult]) => {
      if (!active) return;
      setSettings(nextSettings);
      setGlossary(glossaryResult.glossary);
      setChanged(false);
    }).catch((loadError) => {
      if (active) setError(errorText(loadError, 'Không thể tải thiết lập dịch.'));
    }).finally(() => {
  ...
  const recoverConflict = (mutationError: unknown): boolean => {
    ...
    setSettings((current) => ({
      ...current,
      stylePreset: recovered.settings.stylePreset,
      contextRevision: recovered.settings.contextRevision,
    }));
  ...
  const changeStyle = async (stylePreset: TranslationStyle) => {
    ...
    setConflictMessage('');
    try {
      const next = await services.updateTranslationStyle(projectId, settings.contextRevision, stylePreset);
      setSettings(next);
      setChanged(true);
    } catch (mutationError) {
      if (!recoverConflict(mutationError)) setError(errorText(mutationError, 'Không thể lưu phong cách dịch.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const startCreate = () => {
    if (glossary.length >= 200) {
      setError('Dự án đã đạt giới hạn 200 thuật ngữ.');
      return;
    }
    setEditingEntryId(null);
  ...
  const saveDraft = async () => {
    ...
    const validation = validateGlossaryDraft(draft);
    if (validation) {
      setError(validation);
      return;
    }
    ...
    try {
      const result = editingEntryId
        ? await services.updateGlossaryEntry(projectId, editingEntryId, settings.contextRevision, draft)
        : await services.createGlossaryEntry(projectId, settings.contextRevision, draft);
      setSettings((current) => ({
        ...current,
        stylePreset: result.context.style,
        contextRevision: result.contextRevision,
      }));
      setGlossary(result.context.glossary);
      setDraft(null);
      setEditingEntryId(null);
      setChanged(true);
    } catch (mutationError) {
      if (!recoverConflict(mutationError)) setError(errorText(mutationError, 'Không thể lưu thuật ngữ.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const removeEntry = async (entry: GlossaryEntryDto) => {
    ...
    setConflictMessage('');
    try {
      const result = await services.deleteGlossaryEntry(projectId, entry.id, settings.contextRevision);
      setSettings((current) => ({
        ...current,
        stylePreset: result.context.style,
        contextRevision: result.contextRevision,
      ...
      if (!recoverConflict(mutationError)) setError(errorText(mutationError, 'Không thể xóa thuật ngữ.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <TranslationSettingsPanelView
      ...
      onCancelEdit={() => { setDraft(null); setEditingEntryId(null); setError(''); }}
