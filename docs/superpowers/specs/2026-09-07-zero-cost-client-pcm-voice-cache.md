...
For `vi`, mirror the completed voice result onto the legacy `segments` row only when the mirrored segment version/translation remains compatible. The modern target-language export path must not depend on this mirror.
