...
Production runtime remains **UNQUALIFIED** until that real supported Sync provider/media fixture completes end-to-end from a normal dubbed export through short-lived provider media delivery, Sync generation, canonical R2 visual publication, persisted completion state, and owner download. After that evidence is reviewed, the operator may explicitly set `SYNC_LIPSYNC_QUALIFIED=true`; until then the UI/API/Workflow remain fail-closed even if the Sync credential exists.
