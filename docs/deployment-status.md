...
Target translations, dubbed audio and export variants are persisted independently. Target voice artifacts are scoped under `projects/{projectId}/voices/{targetLanguage}/...`; subtitle artifacts are scoped under `projects/{projectId}/subtitles/{targetLanguage}/{exportId}.srt`. Dubbed variants use the zero-container PCM/Stream publisher and retain concrete target/export identity rather than overwriting sibling targets. Non-Vietnamese work never overwrites legacy Vietnamese fields; a completed `vi` target variant alone may mirror into `projects.export_object_key`.
