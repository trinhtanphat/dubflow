...
True `separated_background` remains fail-closed. Source now wires the pinned Demucs adapter behind explicit `GET`/`POST` preparation and a dedicated separation Workflow. `SEPARATION_RUNTIME_QUALIFIED` defaults to `false`, and the Workers Builds production config strips container and Durable Object deployment, so canonical production cannot start separation and remains **UNQUALIFIED**. Studio requires a qualified provider plus a durable current-generation `Ready` background stem before separated-background export. Export itself only reuses `latestCompleted` and never starts separation or silently downgrades to ducking or dubbed-only output. When a qualified runtime explicitly performs preparation, `dialogue_separation_second` is recorded idempotently and durable stem reuse does not charge the same provider work again.
...
A GREEN Phase 4D source CI, build, migration guard, FFmpeg contract test, Studio test, or Wrangler dry-run does not prove real separated-background production behavior. Production runtime remains **UNQUALIFIED** for separated background until the pinned provider is qualified with a real deployed authorized media fixture proving explicit preparation, durable stem persistence/reuse, final rendering, retrieval, source-generation invalidation, and cancellation/failure boundaries. The detailed qualification contract is in `docs/phase4d-background-prepare-qualification.md`.

## Phase 4E optional visual lip-sync qualification

Phase 4E is **source/CI qualification only** for optional visual lip-sync. Migration `0012_visual_lipsync.sql` advances the repository source readiness target to schema revision **12**, persists visual processing state on canonical project exports, and adds short-lived provider-media grant authority. Grant tokens remain hash-only in D1, are bound to exact project-scoped objects, and expire after the bounded provider-access window.

The normal dubbed artifact `projects/{projectId}/exports/{targetLanguage}/{exportId}.mp4` remains canonical first and is preserved as the user-visible fallback if visual processing fails. A successful visual result is published separately as `projects/{projectId}/exports/{targetLanguage}/{exportId}.lipsync.mp4`. Owner retrieval of the visual artifact is fail-closed until the persisted lip-sync state is completed and the exact canonical visual key matches the export attempt.

Sync Labs remains an optional configured provider. Source CI, mock-provider tests, Wrangler dry-run, schema checks, screenshots, or a successful deployment do not prove real Sync execution. Production runtime remains **UNQUALIFIED** until a real supported Sync provider/media fixture completes end-to-end from a normal dubbed export through short-lived provider media delivery, Sync generation, canonical R2 visual publication, persisted completion state, and owner download.

The historical live evidence at the top of this document remains the last separately qualified production state; this feature branch must not preclaim a live schema revision 12 rollout. After merge, Cloudflare Workers Builds remains the only production deployment lane. GitHub Actions stays CI-only and performs Wrangler validation only with `--dry-run`.
...
Final provider/media runtime qualification still requires real supported fixtures: Deepgram for diarization, configured contextual translation for style/glossary behavior, ElevenLabs/FFmpeg for final export, an authorized IVC sample for cloning, for Phase 4C at least two distinct supported target languages through translation, TTS, render, retrieval and concrete export sharing, for Phase 4D a separately qualified dialogue-separation provider before `separated_background` can be called production-qualified, and for Phase 4E a real supported Sync provider/media fixture through canonical visual publication and owner retrieval. The Phase 4E source also requires a successful normal Workers Builds rollout to readiness schema revision 12 before its API/schema layer can be called live-qualified. Until those live fixtures and rollout evidence succeed, those runtime capabilities remain **UNQUALIFIED** rather than PASS.
