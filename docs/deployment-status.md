...
Source support for final dubbed export exists through the ElevenLabs + FFmpeg export workflow, including source-level per-speaker voice routing. Phase 4B also adds source support for explicit-consent ElevenLabs Instant Voice Clone enrollment, but this remains distinct from production-runtime qualification. Visual lip-sync rendering remains capability-gated and unqualified.
...
ElevenLabs `requires_verification=true` maps to `verification_required`; that clone is persisted for lifecycle/deletion purposes but is not assignable. Only `ready` managed clones may be assigned to speakers, and assignment reuses the existing speaker voice update path so previously generated dubbed audio and stale project exports are invalidated before a re-render. Provider deletion is fail-closed: assigned speakers are cleared through the same invalidation path and a provider deletion failure is not reported as deleted.

The capability API now distinguishes normal TTS preview from managed clone enrollment through `cloneEnrollment: { provider: 'elevenlabs', mode: 'ivc', available }`. Studio surfaces consent, sample selection, creating, verification-required, ready, failed and deleting states truthfully; assignment remains an explicit user action and is never automatic after enrollment.

Professional Voice Clone creation/training/verification orchestration remains out of Phase 4B scope. Phase 4B is **source/CI qualification only** and does not prove a production voice-cloning fixture. Production deployment remains **manual-only**, no Phase 4B production deploy is performed by this qualification work, and production runtime remains **UNQUALIFIED** until the existing Cloudflare Container credential and real authorized ElevenLabs/media fixture gates pass.
