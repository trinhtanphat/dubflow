...
CI contains regression tests that fail if a GitHub production deployment workflow is reintroduced or if CI starts performing a non-dry-run Wrangler deploy. CI also locks the Workers Builds Containers token requirement into this policy so the documented remediation remains the single supported path.
