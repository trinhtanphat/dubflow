...
- Source languages: auto / Chinese / English / Japanese / Korean. Vietnamese remains the legacy/default target; Phase 4C adds a bounded export target set of exactly Vietnamese (`vi`), English (`en`), Japanese (`ja`), Korean (`ko`) and Chinese (`zh`).
...
- Persisted speaker display names and per-speaker ElevenLabs voice IDs. Changing a speaker voice invalidates only that speaker's generated dubbed clips plus stale target exports; renaming alone does not discard valid audio.
...
- Phase 4C source support adds target-aware translations, dubbed audio and export variants for `vi/en/ja/ko/zh`, batch export of 1-4 distinct targets, independent partial-success state, target-scoped R2 objects, owner download of one concrete variant, and concrete-variant bearer sharing. Completed Vietnamese variants preserve the legacy `projects.export_object_key` mirror for backward compatibility.
- Phase 4C Studio exposes a compact cloud-only target/export panel with explicit per-language state, concrete MP4 download and share actions. Legacy single-Vietnamese topbar export/share behavior remains available.
- Phase 3B usage accounting remains authoritative for translation, TTS and render work. `RATE_LIMIT_BATCH_EXPORT` is admission/abuse control only and does not create credits, pricing or a parallel billing ledger.
...
Phase 4C is likewise **source/CI qualification only**. Automated tests qualify the bounded five-language authority, 1-4-target batch admission, target-specific persistence/invalidation, partial-success aggregation, concrete export download/share behavior, legacy Vietnamese compatibility and Studio controls. Source CI does not prove that real configured translation, ElevenLabs TTS and FFmpeg rendering have succeeded for multiple languages. A later runtime qualification requires a real authorized media/provider fixture with at least two distinct supported targets end-to-end. Phase 4C does **not** trigger production deployment; this implementation stops at source/CI and post-merge verification.
...
- Do not claim Phase 4C multi-language runtime PASS from source CI, Wrangler dry-run, screenshots or a deployment alone; at least two real supported target-language fixtures must complete translation, TTS, render and retrieval/share qualification.
