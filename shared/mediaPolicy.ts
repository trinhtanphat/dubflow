...
export function validateMediaFile(file: MediaFileLike): MediaValidationResult {
  ...
  const supportedByExtension = (SUPPORTED_VIDEO_EXTENSIONS as readonly string[]).includes(extension);
  const supportedByMime = normalizedType ? supportedMimeTypes.has(normalizedType) : false;
