...
describe('Workers AI ASR', () => {
  it('uses whisper-large-v3-turbo in transcribe mode with VAD and optional language', async () => {
    ...
    expect(ai.calls[0]).toMatchObject({
      ...
      input: {
        audio: { body: 'AQID', contentType: 'video/mp4' },
        task: 'transcribe',
        language: 'zh',
        vad_filter: true,
      },
  ...
  it('wraps direct source media in the Workers AI binding media envelope and honors an explicit media type', async () => {
    const ai = new FakeAI();
    const provider = new WorkersAIAsrProvider(ai);
    const audio = new Uint8Array([1, 2, 3]).buffer;

    await provider.transcribe(audio, { sourceLanguage: 'auto', mediaType: 'video/quicktime' });
