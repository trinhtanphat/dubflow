...
describe('media processing boundary', () => {
  it('requires an uploaded source object', async () => {
    const service = new ProcessService(new Store({ id: 'p', userId: 'u', title: 'x', sourceLanguage: 'zh', targetLanguage: 'vi', status: 'draft' }));
  ...
  it('returns explicit unavailable status instead of fake export completion', async () => {
    const service = new ProcessService(new Store({ id: 'p', userId: 'u', title: 'x', sourceLanguage: 'zh', targetLanguage: 'vi', status: 'ready', sourceObjectKey: 'projects/p/source/x.mp4' }));
